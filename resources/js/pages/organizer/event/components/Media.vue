<template>
    <div class="rounded-2xl border border-slate-100 p-4 mb-4">
        <!-- Section Header -->
        <p class="font-black text-blue-700 mb-3 pb-2" style="border-bottom: 2px solid; border-image: linear-gradient(90deg,#2f9bef,#f59e0b) 1">
            Upload Event Image
        </p>

        <!-- Upload Area -->
        <div class="rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/40 hover:bg-blue-50 grid place-items-center py-8 cursor-pointer relative" @click="triggerFileInput">
            <input type="file" ref="uploadInput" class="hidden" accept="image/*" @change="handleFiles" />
            <Upload class="w-6 h-6 text-blue-500 mb-1" />
            <span class="text-sm font-bold text-slate-600">Upload photo</span>
        </div>

        <!-- Previews -->
        <div v-if="previews.length > 0 || (props.events && props.events.image_object)" class="flex flex-wrap gap-2 mt-3">
            <div v-if="previews.length === 0 && props.events?.image_object" class="relative group">
                <img class="h-20 w-20 rounded-xl object-cover"
                    :src="`${props.appURL}${props.events?.image_object}`" alt="Preview" />
            </div>
            <template v-for="(file, index) in previews" :key="index">
                <div class="relative group">
                    <img v-if="file.type && file.type.startsWith('image/')" :src="file.url" class="h-20 w-20 rounded-xl object-cover" />
                    <div v-else class="text-red-500 text-sm">Unsupported file</div>
                    <button type="button" @click.stop="removePreview(index)"
                        class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition shadow-sm">
                        ✕
                    </button>
                </div>
            </template>
