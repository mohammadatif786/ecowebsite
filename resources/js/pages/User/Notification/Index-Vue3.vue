            ...
            <section v-if="todayNotifications.length > 0" aria-labelledby="today" class="mb-8">
                ...
                <ul role="feed" aria-busy="false" class="mt-2 space-y-2">
                    <NotificationCard v-for="notification in todayNotifications" :key="notification.id"
                        ...
                        :is-selected="selected.has(notification.id)"
                        @toggle-select="toggleSelect(notification.id)" @mark-read="markAsRead(notification.id)"
                        @delete="openDeleteModal(notification.id)" @click="handleNotificationClick(notification)"
...
const handleAcceptInvite = async (notification: Notification) => {
    ...
    // 2. Accept invite + redirect to GoLive. (GoLive page will auto-join as guest via ?join_stream=...)
    try {
