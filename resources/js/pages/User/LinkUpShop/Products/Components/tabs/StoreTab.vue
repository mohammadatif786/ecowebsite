...
import { ArrowLeft, Check, Trash2Icon } from 'lucide-vue-next';
...
const bizFileInput = ref<HTMLInputElement | null>(null);
const vatFileInput = ref<HTMLInputElement | null>(null);
const hasBusinessLicenseFile = ref(false);
const hasVatCertificateFile = ref(false);
const businessLicensePreview = ref<string | null>(null);
const vatCertificatePreview = ref<string | null>(null);
import { toast } from 'vue-sonner';

const handleFile = (event: any, field: 'business_license_file' | 'vat_certificate_file') => {
    const file = event.target.files[0];
    if (file) {
        form[field] = file;
        if (field === 'business_license_file') hasBusinessLicenseFile.value = true;
        if (field === 'vat_certificate_file') hasVatCertificateFile.value = true;
        const preview = file.type.startsWith('image/') ? URL.createObjectURL(file) : null;
        if (field === 'business_license_file') businessLicensePreview.value = preview;
        if (field === 'vat_certificate_file') vatCertificatePreview.value = preview;
    }
};
...
const selectStore = (merchant: any) => {
    ...
    // Legacy stores predate these columns and defaulted to pickup only.
    form.offers_pickup = merchant.offers_pickup ?? true;
    form.offers_delivery = merchant.offers_delivery ?? false;
    form.business_license = merchant.business_license || '';
    form.vat_certificate = merchant.vat_certificate || '';
    form.business_license_file = null;
    form.vat_certificate_file = null;
    hasBusinessLicenseFile.value = Boolean(merchant.business_license_file);
    hasVatCertificateFile.value = Boolean(merchant.vat_certificate_file);
    businessLicensePreview.value = merchant.business_license_file || null;
    vatCertificatePreview.value = merchant.vat_certificate_file || null;
...
const validateStore = () => {
    ...
    if (!form.st_name?.trim()) errors.st_name = 'Store name is required.';
    if (!form.st_owner?.trim()) errors.st_owner = 'Owner name is required.';
    ...
    if (selectedProductId.value) {
        ...
        form.post(route('frontend.update.store', selectedProductId.value), {
            ...
            onSuccess: () => {
                ...
                emit('back');
                emit('close');
                if (window.toast) window.toast(form.st_name + ' Store Updated ');
            },
            onError: async () => {
                await fetchCountries();
            }
        ...
        form.post(route('frontend.save.store'), {
            ...
            onSuccess: () => {
                ...
                emit('back');
                emit('close');
                if (window.toast) window.toast(form.st_name + ' Store Created ');
            },
            onError: async () => {
                await fetchCountries();
            }
...
const clearSelection = () => {
    ...
    form.reset();
    hasBusinessLicenseFile.value = false;
    hasVatCertificateFile.value = false;
    businessLicensePreview.value = null;
    vatCertificatePreview.value = null;
};
...
const confirmDelete = (storeId: number) => {
    form.delete(route('frontend.destroy.store', storeId), {
        onSuccess: () => {
            ...
            if (window.toast) window.toast('Store Deleted');
        }
    });
};
...
<template>
    <div class="grid gap-3" v-if="props.activeTab === 'store'">

        <!-- list of store -->

        <div v-if="props.merchants && props.merchants.length > 0" class="mb-4   overflow-hidden">
            <div class="font-black mb-2">My Store (Select to Edit)</div>
            <div class="flex gap-3 overflow-x-auto pb-3 custom-scrollbar">
                ...
                <!-- merchant items -->
                <div v-for="merchant in props.merchants.filter(m => m.merchant_type === 'store')"
        ...
        <div class="grid md:grid-cols-2 gap-2">
            <div>
                <div class="font-black text-xs uppercase tracking-wider text-slate-500">Store Name</div>
            ...
            <div>
                <div class="font-black text-xs uppercase tracking-wider text-slate-500">Owner Name</div>
        ...
        <div class="grid md:grid-cols-3 gap-2">
            <div>
                <div class="font-black text-xs uppercase tracking-wider text-slate-500">Country</div>
            ...
            <div>
                <div class="font-black text-xs uppercase tracking-wider text-slate-500">State</div>
            ...
            <div>
                <div class="font-black text-xs uppercase tracking-wider text-slate-500">City</div>
        ...
        <div>
            <div class="font-black text-xs uppercase tracking-wider text-slate-500">Fulfillment Options</div>
            <div class="grid grid-cols-2 gap-3 mt-1">
                <label
                    class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-white cursor-pointer hover:bg-slate-50 transition">
                    <input type="checkbox" v-model="form.offers_pickup"
                        class="w-5 h-5 rounded-lg text-lkblue focus:ring-lkblue border-slate-300" />
                    <span class="text-sm font-bold flex items-center gap-2">🏪 Offers Pickup</span>
                </label>
                <label
                    class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-white cursor-pointer hover:bg-slate-50 transition">
                    <input type="checkbox" v-model="form.offers_delivery"
                        class="w-5 h-5 rounded-lg text-lkblue focus:ring-lkblue border-slate-300" />
                    <span class="text-sm font-bold flex items-center gap-2">🚚 Offers Delivery</span>
                </label>
            </div>
        </div>

        <div>
            <div class="font-black text-xs uppercase tracking-wider text-slate-500">Pickup Locations (comma-separated)
            </div>
        ...
        <div>
            <div class="font-black text-xs uppercase tracking-wider text-slate-500">Business Documents</div>
            <div class="grid grid-cols-2 gap-3 mt-1">
                <div class="space-y-2">
                    <img v-if="businessLicensePreview && !businessLicensePreview.toLowerCase().endsWith('.pdf')"
                        :src="businessLicensePreview" alt="Business license preview"
                        class="h-24 w-full rounded-xl border border-slate-200 object-cover" />
                    <input class="input" v-model="form.business_license" placeholder="Business License #" />
                    <input type="file" ref="bizFileInput" class="hidden"
                        @change="e => handleFile(e, 'business_license_file')" />
                    <button @click="bizFileInput?.click()" type="button"
                        :class="['w-full py-2 px-3 rounded-xl border-2 border-dashed text-xs font-black flex items-center justify-center gap-2 transition', hasBusinessLicenseFile ? 'border-emerald-200 bg-emerald-50 text-emerald-600' : 'border-slate-200 text-slate-400 hover:bg-slate-50']">
                        <i :data-lucide="hasBusinessLicenseFile ? 'check' : 'upload'" class="w-4 h-4"></i>
                        {{ hasBusinessLicenseFile ? 'Uploaded' : 'Upload document' }}
                    </button>
                </div>
                <div class="space-y-2">
                    <img v-if="vatCertificatePreview && !vatCertificatePreview.toLowerCase().endsWith('.pdf')"
                        :src="vatCertificatePreview" alt="VAT certificate preview"
                        class="h-24 w-full rounded-xl border border-slate-200 object-cover" />
                    <input class="input" v-model="form.vat_certificate" placeholder="VAT Certificate #" />
                    <input type="file" ref="vatFileInput" class="hidden"
                        @change="e => handleFile(e, 'vat_certificate_file')" />
                    <button @click="vatFileInput?.click()" type="button"
                        :class="['w-full py-2 px-3 rounded-xl border-2 border-dashed text-xs font-black flex items-center justify-center gap-2 transition', hasVatCertificateFile ? 'border-emerald-200 bg-emerald-50 text-emerald-600' : 'border-slate-200 text-slate-400 hover:bg-slate-50']">
                        <i :data-lucide="hasVatCertificateFile ? 'check' : 'upload'" class="w-4 h-4"></i>
                        {{ hasVatCertificateFile ? 'Uploaded' : 'Upload document' }}
                    </button>
                </div>
            </div>
            <p class="text-[10px] text-slate-400 mt-2 leading-relaxed">
                Optional to start. Government registration becomes required once your store crosses $10,000.00 in
                lifetime sales.
            </p>
        </div>
        ...
        <div class="flex gap-2 pt-2">
            <button
                ...
                style="background: linear-gradient(135deg, #3b82f6 0%, #10b981 100%)" @click="handleSaveStore">
                ✓ {{ selectedProductId ? 'Update Store' : 'Save Store' }}
            </button>
            <button v-if="selectedProductId" type="button"
                class="flex-1 rounded-2xl border-2 border-rose-200 bg-rose-50 px-4 py-4 font-black text-rose-600 transition hover:bg-rose-100 flex items-center justify-center gap-2"
                :disabled="form.processing" @click="deleteStore">
                <Trash2Icon class="w-5 h-5" />
                Delete
            </button>
            <button
                class="flex-1 rounded-2xl py-4 px-5 font-black border-2 border-slate-200 hover:bg-slate-50 transition flex items-center justify-center"
                @click="$emit('back')">
                ← Back
            </button>
        </div>
