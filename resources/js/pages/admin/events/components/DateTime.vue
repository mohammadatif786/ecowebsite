<template>
    <div class="form-section">
        ...
        <button type="button" class="add-section-btn" @click="toggleSection(0)" v-if="!showFields[0]">+</button>
        <button type="button" class="add-section-btn" @click="toggleSection(0)" v-else>-</button>

        <p v-if="!showFields[0]" class="section-overview">
            Specify the start and end dates and times for your event to ensure attendees can plan accordingly.
        </p>

        <div v-if="showFields[0]" class="form-fields">
            <h3>Type of event</h3>
            <div class="event-type-tabs">
                <div class="event-type-tab" :class="{ active: form.eventType === 'single' }"
                    @click="form.eventType = 'single'">
                    <span>??</span> <span class="mx-2">Single event</span>
                    <span>For events that happen once</span>
                </div>

                <div class="event-type-tab" :class="{ active: form.eventType === 'recurring' }"
                    @click="form.eventType = 'recurring'">
                    <span>??</span> Recurring event
                    <span class="new-badge mx-2">New</span>
                    <span>For timed entry and multiple days</span>
                </div>
            </div>


            <!-- Single event options -->
            <div v-if="form.eventType === 'single'" id="single-event-options">
                <h3>Date and time</h3>
                <div class="date-time-layout">
                    <div class="form-group date-input">
                        <label for="event-date">Date</label>
                        <input type="date" id="event-date" v-model="form.singleDate">
                    </div>
                    <div class="form-group time-input">
                        <label for="start-time">Start time</label>
                        <input type="time" id="start-time" v-model="form.singleStartTime">
                    </div>
                    <div class="form-group time-input">
                        <label for="end-time">End time</label>
                        <input type="time" id="end-time" v-model="form.singleEndTime">
                    </div>
                </div>
            </div>

            <!-- Recurring event options -->
            <div v-if="form.eventType === 'recurring'" id="recurring-event-options" class="recurring-options">
                <h3>Recurring Schedule</h3>
                <div class="form-group">
                    <label for="recurrence-pattern">Recurrence Pattern</label>
                    <select id="recurrence-pattern" v-model="form.recurrPattern">
                ...
                <div class="date-time-layout">
                    <div class="form-group date-input">
                        ...
                        <input type="date" id="start-date" v-model="form.recurrStartDate">
                    </div>
                    <div class="form-group date-input">
                        <label for="end-date">End Date</label>
                        <input type="date" id="end-date" v-model="form.recurrEndDate">
                    </div>
                </div>
            </div>
        </div>
