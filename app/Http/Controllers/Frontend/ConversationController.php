...
class ConversationController extends Controller
{
    ...
    public function index($toUserId, Request $request)
    {
        $query = Message::where(function ($query) use ($toUserId) {
            $query->where('from_user_id', Auth::id())
                ->where('to_user_id', $toUserId);
        })->orWhere(function ($query) use ($toUserId) {
        ...
        // Get latest 50, reverse to chronological ASC for display, and reindex keys
        $messages = $query
            ->latest()
            ->take(50)
            ->get()
            ->reverse()
            ->values()   // reindex to 0..n so the frontend receives a real array
            ->toArray();
        $receiver = User::findOrFail($toUserId);
    ...
    public function store(Request $request, $toUserId)
    {
        ...
        return redirect()->route('frontend.user.start.chat', ['slug' => $returnData->name, 'user' => $returnData->uid]);
    }
}
